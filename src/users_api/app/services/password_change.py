...
class PasswordChangeService:
    ...
    async def change_password(
        ...
        # transaction commits: CA.3 is the security guarantee and it is made
        # to hold even if something below fails. The one thing that can still
        # fail below is the mail: with ConsoleEmailSender it never does, but a
        # real provider can, and if it does the password update rolls back
        # while the revocation stands, so every session ends without the
        # password having changed. Worth revisiting once a real provider with
        # retries lands.
