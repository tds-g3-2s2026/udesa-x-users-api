...
- El código se organiza por feature, no por capa: `src/users_api/features/<feature>/` con su `router.py`, `service.py`, `schemas.py`, `domain.py` y `repositories.py`. Es la misma forma que usa `udesa-x-mobile` (`src/features/auth/`), así que los dos repos se leen igual y una feature se puede mover entera. Lo transversal vive en `core/` (`config`, `db`, `errors`, `security`, `ports`, `deps`). Una feature puede depender de `core/`; entre features, solo `password_reset` depende de `auth`, porque el reset opera sobre cuentas y sesiones que son de `auth`.
- **Los servicios no importan SQLAlchemy ni Redis.** Piden lo que necesitan a través de interfaces: `repositories.py` de cada feature para sus datos, `core/ports.py` para correo, límites de uso y revocación de sesiones. Las implementaciones viven en `infrastructure/` y el único módulo que las nombra es `core/deps.py`. Al agregar una capacidad nueva: la interfaz va del lado de la feature, la clase concreta en `infrastructure/`, y el cable en `deps.py`.
